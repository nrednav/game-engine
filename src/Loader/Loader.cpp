...
void Loader::bindIndicesBuffer(std::vector<int>& indices)
{
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, indices.size() * sizeof(int), &indices[0],
		GL_STATIC_DRAW);
}
...
int Loader::loadTexture(std::string fileName, bool repeat)
{
	...
	stbi_uc* imageData = stbi_load(filePath.c_str(), &width, &height, &componentCount, 4);
...
int Loader::loadCubeMap(std::vector<std::string>& textureFiles)
{
	...
	for (int i = 0; i < textureFiles.size(); i++) {
		...
		glTexImage2D(
			...
			0, GL_RGBA, data->getWidth(), data->getHeight(), 0, GL_RGBA,
...
TextureData* Loader::decodeTextureFile(std::string fileName)
{
	...
	stbi_uc* imageData = stbi_load(
		filePath.c_str(), &width, &height, &nComponents, STBI_rgb_alpha
	);
