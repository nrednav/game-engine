...
{
	glBindAttribLocation(this->programID, attribute, (const GLchar*)variableName.c_str());
}

int ShaderProgram::getUniformLocation(std::string uniformName)
{
	return glGetUniformLocation(this->programID, (const GLchar*) uniformName.c_str());
}

bool ShaderProgram::hasErrors(unsigned int shaderID, int type) {
	int success;
	char infoLog[512];
	std::string shaderType = (type == GL_VERTEX_SHADER)
		? "VERTEX" : "FRAGMENT";
