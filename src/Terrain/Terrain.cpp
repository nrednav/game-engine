...
Terrain::Terrain(float gridX, float gridZ, Loader* loader,
...
RawModel* Terrain::generateTerrain(Loader* loader,
  ...
  stbi_uc* imageData = stbi_load(hmPath.c_str(), &imageWidth, &imageHeight,
                                 &nComponents, STBI_rgb);
  ...
  for (int i = 0; i < TERRAIN_VERTEX_COUNT; i++) {
    for (int j = 0; j < TERRAIN_VERTEX_COUNT; j++) {
      ...
      textureCoords[vertexPointer * 2 + 1] =
          (float)i / ((float)lastTerrainIndex);
...
glm::vec3 Terrain::calculateNormal(const int& x, const int& z,
                                   stbi_uc* imageData, const int& imageWidth) {
